...
**Тема:**  "Разработке информационной системы для автоматизации и управления процессом 3D-печати"
...
7. **Уведомление об оканчании пластика**: Система должна уведомлять администратора через веб-интерфейс, когда запас любого материала заканчивается.
...
15. **Сбор и визуализация статистики**: Система должна накапливать данные и предоставлять отчеты и графики по следующим метрикам:
...
![Image](https://github.com/user-attachments/assets/126eb2b0-1cb3-4d99-b1ab-b7d9df28a1dd)
...
1. **Время отклика UI**: Большинство операций в пользовательском интерфейсе (навигация, открытие форм) должны выполняться менее чем за 1 секунду. Максимально допустимое время отклика на любое действие пользователя 2 секунды.
2. **Время анализа модели**: Автоматический расчёт параметров печати (время, стоимость, объем материала) для стандартной модели в формате 3MF (до 50 МБ) должен выполняться не более чем за 20 секунд.
3. **Частота обновления данных**: Данные телеметрии (прогресс печати, температура) и статусы принтеров в веб-интерфейсе должны обновляться с интервалом не более 3 секунд для обеспечения эффективного мониторинга в реальном времени.
4. **Скорость отображения видеопотока**: Задержка в передаче видеопотока со встроенных камер принтеров до интерфейса пользователя не должна превышать 5 секунд.
5. **Масштабируемость**: Архитектура системы должна позволять управлять парком до 20 3D-принтеров с одного сервера (Orange Pi) без значительной снижения производительности.
6. **Надежность**: Потеря связи с одним из 3D-принтеров не должна влиять на работу системы в целом и на управление другими принтерами. Система должна корректно отрабатывать подобные сбои, помечая принтер как "не в сети" и уведомляя пользователя.
7. **Доступность**: Уровень доступности веб-интерфейса системы должен составлять не менее 99% в рабочее время (например, с 9:00 до 18:00). Плановые технологические простои для обновления ПО допускаются при предварительном уведомлении.
8. **Аутентификация**: Доступ к функциям системы должен быть защищён учётной записью и паролем.
9. **Защита данных**: Конфиденциальные данные (пароли, ключи API) должны храниться в базе данных в зашифрованном виде. Передача всех данных между браузером клиента и сервером (Orange Pi) должна осуществляться по защищенному протоколу HTTPS.
10. **Интуитивность интерфейса**: Интерфейс системы должен быть интуитивно понятен для пользователя с базовыми знаниями в 3D-печати. Основные операции (загрузка модели, запуск печати) должны выполняться не более чем в 3 клика.
11. **Обратная связь**: Система должна предоставлять понятные сообщения об ошибках (например, "Недостаточно материала ABS", "Принтер 'bambulab' не отвечает").
12. **Логирование**: Система должна вести логи всех ключевых событий: запуск и завершение печати, ошибки оборудования, изменения в остатках материалов, действия пользователей по управлению принтерами. Логи должны храниться не менее 30 дней для последующего анализа и отладки.
13. **Мониторинг ресурсов**: В системе должен быть мониторинг собственной работоспособности (нагрузка на CPU и RAM сервера Orange Pi, свободное место на диске) с предупреждениями администратору при критических значениях.